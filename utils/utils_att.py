...
from models.model_att import EncoderRNN, AttnDecoderRNN
...
def load_objects(obj1, obj2):
    with open(os.path.join(os.getcwd(), 'artifacts', 'att_model', f'{obj1}.dill'), 'rb') as f:
        object1 = dill.load(f)
    with open(os.path.join(os.getcwd(), 'artifacts', 'att_model', f'{obj2}.dill'), 'rb') as f:
        object2 = dill.load(f)
...
load_checkpoint(os.path.join(os.getcwd(), 'artifacts', 'att_model', 'checkpoint_epoch_100.pt'), encoder, decoder)
